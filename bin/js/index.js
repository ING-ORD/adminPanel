    ...
    for (var i = 0;i<lesson.length;i++){
      data_text = "";
        if (i in data){
            ...
            lesson[i].querySelector(".wrap-days-items").innerHTML = CreateElementHTML("div",data_text,{"class":"days-items"}).outerHTML;
        }
    }
};
...
document.querySelector(".wrap-content-block").addEventListener("click",function(e){
  if(e.target.classList.contains("days-items__tool")){
      if (this.parentElement.querySelector(".days-items__input").value != ""){
          this.parentElement.querySelector(".days-items__input").outerHTML = CreateElementHTML("div",this.parentElement.querySelector(".days-items__input").value,{"class":"days-items__text"}).outerHTML;
